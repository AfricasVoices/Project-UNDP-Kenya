...
if __name__ == "__main__":
    ...
    for path in traced_data_paths:
        ...
        for td in data:
            ...
            # UNDP-Kenya is exporting contacts from a project with incomplete labels. If a participant has any data
            # that hasn't been reviewed, then we shouldn't include them in the advert list, just in case that
            # unreviewed data was an opt-out.
            nr = False
            for key in {"age_category", "gender", "constituency", "county"}:
                if td[key] == "NR":
                    nr = True

            for key in td:
                if key.endswith("_NR") and td[key] == Codes.MATRIX_1:
                    nr = True
